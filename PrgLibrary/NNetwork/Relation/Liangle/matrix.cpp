...
Vector Matrix::dot(const Vector& vector) const
{
    Vector new_vector(M);
    double result = 0.0;
...
Vector Matrix::T_dot(const Vector& vector) const
{
    Vector new_vector(N);
    double result = 0.0;
...
Matrix Matrix::T_join(const Vector& vector) const
{
    Matrix new_matrix(N, M);

    for(std::size_t i = 0; i < N; ++i)
        for(std::size_t j = 0; j < M; ++j)
            new_matrix.data[i][j] = data[j][i] * vector(i);
