...
NNetwork::NNetwork(int a, int b, int c)
...
void NNetwork::setActivationFunction(double (*func)(double), double (*func_deriv)(double))
{
    f_act = func;
    f_act_deriv = func_deriv;
}

Vector NNetwork::feedforward(const Vector& in) const
{
    Vector H_b((W_ab.T_dot(in) + B_b).modify(f_act));
    Vector H_c((W_bc.T_dot(H_b) + B_c).modify(f_act));

    return H_c;
}

void NNetwork::train(const Matrix& in, const Matrix& out, double learn_rate, double loss_rate)
{
    Vector H_a;
    Vector H_b;
    Vector H_c;
    ...
    for(int epoch = 0; loss > loss_rate; ++epoch)
    {
        ...
        DW_ab = H_a.tensordot_T(DH_b).join_T(W_bc.dot(H_c.join(Y_c)));
        DW_bc = H_b.tensordot_T(DH_c).join_T(Y_c);
        DB_b = DH_b.join(W_bc.dot(H_c.join(Y_c)));
        DB_c = DH_c.join(Y_c);
...
double NNetwork::mean(const Matrix& y_true, const Matrix& y_pred) const
{
    double result = 0.0;
