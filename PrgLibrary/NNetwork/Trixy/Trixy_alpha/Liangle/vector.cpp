...
Vector& Vector::resize(std::size_t n)
{
    delete[] data;
...
Vector& Vector::fill(double (*generator)())
{
    for(std::size_t i = 0; i < N; ++i)
        data[i] = generator();
...
Vector Vector::dot(const Matrix& matrix) const
{
...
Vector Vector::dot_T(const Matrix& matrix) const
{
    Vector new_vector(matrix.row());
    double result = 0.0;
...
Matrix Vector::join(const Matrix& matrix) const
{
    ...
    for(std::size_t i = 0; i < matrix.row(); ++i)
        for(std::size_t j = 0; j < matrix.col(); ++j)
            new_matrix(i, j) = data[j] * matrix(i, j);

    return new_matrix;
}

Matrix Vector::tensordot(const Vector& vector) const
{
    Matrix new_matrix(vector.N, N);

    for(std::size_t i = 0; i < vector.N; ++i)
        for(std::size_t j = 0; j < N; ++j)
            new_matrix(i, j) = data[j] * vector.data[i];
