...
Matrix NNetwork::feedforward(const Matrix& in) const
{
    Matrix y_pred = in;
    for(std::size_t n = 0; n < in.row(); ++n)
        for(std::size_t i = 0; i < N; ++i)
            y_pred(n) = (y_pred(n).dot(W[i]) + B[i]).produce(F);

    return y_pred;
}

void NNetwork::trainStochastic(const Matrix& in, const Matrix& out, double learn_rate, std::size_t epoch_rate)
{
    ...
    for(std::size_t epoch = 1; epoch <= epoch_rate; ++epoch)
    {
        ...
        for(std::size_t i = 0; i < N; ++i)
        {
            ...
            if (i < N - 1)
            {
                for(std::size_t n = N - 1; n > i + 1; --n)
                    theta = theta.dot_T( DH[n].join(W[n]) );

                theta = W[i + 1].dot_T( DH[i + 1].join(theta) );
            }

            DW[i] = DH[i].tensordot(H[i]).join(theta);
        ...
        if(epoch % 100 == 0)
        {
            //std::cout << "Epoch " << epoch << " loss: " << loss(in, out) << '\n';
...
void NNetwork::test(const Matrix& in, const Matrix& out) const
{
    for(std::size_t i = 0; i < in.row(); ++i)
        std::cout << "<" << i << "> " << feedforward(in(i)) << " : " << out(i) << '\n';
}
